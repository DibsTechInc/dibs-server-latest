...
WITH super_attendee AS (
  SELECT DISTINCT
      ...
      a."attendeeID",
      all_studios."mainTZ",
      COALESCE(e.mbclassid, e.zfclass_id) AS classid,
      COALESCE(e.mblocationid, e.zfsite_id) AS locationid,
      e.eventid,
      e.start_date,
      e.seats,
      dt.userid,
   ...
   INNER JOIN events AS e
     ON a."studioID" = COALESCE(e.mbstudioid, e.zfstudio_id)
        AND a."classID" = COALESCE(e.mbclassid, e.zfclass_id)
...
DROP TRIGGER new_data_attendee;
