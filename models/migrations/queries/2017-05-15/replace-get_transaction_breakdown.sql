DROP FUNCTION get_transaction_breakdown(transid INTEGER);
   ...
   transaction_after_credits AS (
    ...
    transaction_dibs_fee AS (
      SELECT
        dt.id,
        ROUND((
          dts.fee_rate::NUMERIC * (t1.amount_after_credits::NUMERIC - t2.stripe_fees::NUMERIC - COALESCE(dt.tax_amount, 0.)::NUMERIC)
        ), 2)::FLOAT AS dibs_fee
      FROM dibs_transactions dt
