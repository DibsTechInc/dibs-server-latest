BEGIN;
DROP MATERIALIZED VIEW IF EXISTS metric_tables;
CREATE MATERIALIZED VIEW metric_tables AS (
  WITH visits_this_year_to_now AS (
    ...
    SELECT DISTINCT
      ...
      FROM
        studio_with_locations
      INNER JOIN unique_visits_this_year_to_now
      ...
      FROM
        studio_with_locations
      INNER JOIN unique_visits_this_month_to_now
      ...
      FROM
        studio_with_locations
      INNER JOIN unique_visits_today_to_now
      ...
      FROM
        studio_with_locations
      INNER JOIN unique_visits_last_month
      ...
      FROM
        studio_with_locations
      INNER JOIN unique_visits_last_month_to_day
  ...
  SELECT
    COALESCE(visits_today_to_now_counts.visit_date_trunc, DATE_TRUNC('day', timezone(studio_with_locations."mainTZ", timezone('utc', NOW())))) as visit_date_trunc,
    studio_with_locations.studioid,
    studio_with_locations.locationid,
    studio_with_locations.source,
    GREATEST(visits_this_year_to_now_counts.count,0) as visits_year_to_now,
    GREATEST(visits_this_year_to_now_counts.revenue, 0) as revenue_year_to_now,
    GREATEST(visits_this_year_to_now_counts.revenue / unique_visits_this_year_to_now_counts.count, 0) AS revenue_per_visitor_this_year,
    GREATEST(visits_this_month_to_now_counts.count, 0) as visits_month_to_now,
    GREATEST(visits_this_month_to_now_counts.revenue, 0) as revenue_month_to_now,
    GREATEST(visits_this_month_to_now_counts.revenue / unique_visits_this_month_to_now_counts.count, 0) AS revenue_per_visitor_this_month,
    GREATEST(visits_today_to_now_counts.count, 0) as visits_today,
    GREATEST(visits_today_to_now_counts.revenue, 0) as revenue_today,
    GREATEST(visits_today_to_now_counts.revenue / unique_visits_today_to_now_counts.count, 0) AS revenue_per_visitor_today,
    GREATEST(unique_visits_this_year_to_now_counts.count, 0) as unique_visits_this_year,
    GREATEST(unique_visits_this_month_to_now_counts.count, 0) as unique_visits_this_month,
    GREATEST(unique_visits_today_to_now_counts.count, 0) as unique_visits_today,
    GREATEST(visits_last_month_counts.count, 0) as visits_last_month,
    GREATEST(unique_visits_last_month_counts.count, 0) as unique_visits_last_month,
    GREATEST(visits_last_month_counts.revenue, 0) as revenue_last_month,
    GREATEST(visits_last_month_counts.revenue / unique_visits_last_month_counts.count, 0) as revenue_per_visitor_last_month,
    GREATEST(visits_last_month_to_day_counts.count, 0) as visits_last_month_to_day,
    GREATEST(visits_last_month_to_day_counts.revenue, 0) as revenue_last_month_to_day,
    GREATEST(unique_visits_last_month_to_day_counts.count, 0) as unique_visitors_last_month_to_day
