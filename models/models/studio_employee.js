...
module.exports = function linkStudioEmployee(sequelize, DataTypes) {
   ...
   */
  const StudioEmployee = sequelize.define('studio_employee', {
    id: {
    ...
    hooks: {
      ...
      beforeUpdate(user) { /* eslint-disable no-param-reassign */
  ...
  StudioEmployee.createNewEmployee = async function createStudioEmployee({
    ...
    if (isNaN(studioId) || studioId % 1) throw new Error('Please provide a Dibs studio id (integer)');
    const studio = await models.dibs_studio.findById(+studioId);
