...
// use this file to copy the stripe clients for dibs studios based on the client file
...
class StripeClientAdmin {
     ...
     */
    createCustomer({ email, source, managedAccount }) {
        return this.stripe.customers.create({ email, source }, { stripe_account: managedAccount });
    }
     ...
     * @returns {Promise<Object>} resolves response from Stripe
     */
    createCustomerToken({ customerId, managedAccount }) {
     ...
     */
    createCard({ stripeUserId, cardNumber, expiration, cvc, managedAccount, sourceToken }) {
     ...
     */
    deleteCard({ stripeUserId, stripeCardId }) {
     ...
     * @returns {Promise<Object>} resolves response from Stripe
     */
    updateDefaultCard({ stripeUserId, stripeCardId, managedAccount }) {
        return this.stripe.customers.update(stripeUserId, { default_source: stripeCardId }, { stripe_account: managedAccount });
    }
     ...
     * @returns {Promise<Object>} resolves response from Stripe
     */
    chargeCard({ customerId, amount, currency = 'usd', description = '', capture = true, account, statementDescriptor, applicationFee }) {
        const params = {
            amount: StripeClientAdmin.convertAmount(amount),
            capture,
            currency,
            customer: customerId,
            description,
        ...
        if (account) {
            ...
            params.application_fee_amount = StripeClientAdmin.convertAmount(applicationFee);
            // old --> params.application_fee = StripeClientAdmin.convertAmount(applicationFee);
        }
     ...
     * @returns {Promise<Object>} resolves response from Stripe
     */
    completeCharge({ chargeId, amount, account }) {
     ...
     * @returns {Promise<Object>} resolves Stripe response
     */
    async refund(chargeId, { amount, reason = '', stripeAccountId = null } = {}) {
        const { refundType, refundApplicationFee } = await this.getRefundType(stripeAccountId, chargeId);
        const refund = {
            charge: chargeId,
        ...
        if (amount) refund.amount = StripeClientAdmin.convertAmount(amount);

        return refundType === RefundTypes.MANAGED_ACCOUNT
            ? this.stripe.refunds.create(refund, {
                  stripe_account: stripeAccountId
              })
            : this.stripe.refunds.create({ ...refund, reverse_transfer: refundType === RefundTypes.PLATFORM_REVERSE_TRANSFER });
    }
     ...
     * @returns {Promise<Object>} resolves Stripe response
     */
    addBankAccount({ account, country, currency, accountNumber, routingNumber }) {
     ...
     */
    uploadIDFile({ data, name, account }) {
     ...
     * @return {Promise<Object>} resolve Stripe response
     */
    cancelSubscriptionPlan({ stripeSubscriptionId, stripeAccountId }) {
     ...
     * @return {Promise<Object>} resolve Stripe response
     */
    addInvoiceItemToInvoice({ amount, currency, customer, description, invoice }, stripeAccountId) {
     ...
     */
    getStripeFee({ amount, card, currency }) {
    ...
    async findOrCreateManagedAccountCustomer({ user, studio }) {
        ...
        if (!managedAccountCustomerId) {
            ...
            const managedAccountCustomer = await this.createCustomer({
                email: user.email,
                source: token.id,
                managedAccount: studio.stripe_account_id
            });
...
module.exports = StripeClientAdmin;
