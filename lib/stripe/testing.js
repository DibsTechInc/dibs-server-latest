...
const { enrollUserInEvent, checkEventAvailability } = require('../booking/enrollment');
 ...
 */
checkoutLib.checkoutPortalCart = async function checkoutCart({ user, eventid, price }) {
    let charge = false;
    ...
    try {
        ...
        await models.sequelize.transaction(async (sqlTransaction) => {
            ...
            amount += taxAmount;
            const taxWithheldCoeff = Decimal(1).minus(Decimal(1).dividedBy(Decimal(1).plus(taxRate)));
            ...
            t.brand_payment = +Decimal(amount).minus(taxAmount).minus(dibsFee).minus(t.stripe_fee).toDP(2);
        ...
        mc.transactions(
            `Dibs Portal Purchase ${studio.name}`,
            `Portal user ${user.id}, ${user.email} booked event ${eventid} for ${price}. Flash credit ${flashCreditId} was used.`
        );
        await sendTemplatedEmailAsync(user.email, `Your Class at ${emailData.studio.name} is Booked!`, emailData.template, emailData, {
            force: true
        });
