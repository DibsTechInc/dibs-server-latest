...
module.exports = async function getCreditTransactions({ user, dibsStudioId }) {
  const creditTransactions = await models.credit_transaction.findAll({
    where: {
      createdAt: {
        [Op.gte]: moment(Math.max(
          moment('2018-08-01T00:00:00Z'), moment().subtract(3, 'y'))), // 2018-08-01 is when we can guarantee we track reason for credit transactions
      },
      userid: user.id,
