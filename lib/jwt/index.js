...
const signToken = token =>
  jwt.sign(token, JWT_SECRET, { expiresIn: SESSION_EXPIRATION_TIME });
...
const getUnverifiedTokenFromReq = req =>
  ((req.cookies && req.cookies.get) ?
    req.cookies.get(COOKIE_SECRET, { signed: true })
    : getTokenFromReqAuthHeader(req));

const testErrorForInvalidToken = err => Boolean(
  err.constructor.name === 'JsonWebTokenError'
  && /invalid signature|jwt malformed/i.test(err.message));
    ...
    if (err.constructor.name === 'TokenExpiredError') {
      if (NOTIFY_ON_EXPIRED_TOKEN) {
        mc.ops(
          ...
          `Someone is using an expired JWT to access our API.\n\nError Stack: ${err.stack}`);
      }
      return null;
    }
    if (
      testErrorForInvalidToken(err)
      && req.cookies && req.cookies.set
    ) {
...
function storeSessionWithReq(req, signedToken) {
  if (req.cookies && req.cookies.set) {
    req.cookies.set(
      COOKIE_SECRET,
      signedToken,
      { signed: true, maxAge: Date.now() + (SESSION_EXPIRATION_TIME * 1e3), 
        httpOnly: false,
        sameSite: 'None',
        secure: true,
       }
...
function refreshToken(req, token) {
  ...
  Object.keys(TOKEN_KEYS)
        .forEach(key => (refreshedToken[TOKEN_KEYS[key]] = token[TOKEN_KEYS[key]]));
...
async function fetchInstance(sessionType, id) {
  ...
  const instance = cachedJSON ? await models[MODELS[sessionType]].build(cachedJSON,
    {
  ...
  try {
    if (!reqHasAuthHeader) {
      ...
      req.cookies = new Cookies(req, res, {
        keys: [new Buffer(COOKIE_SECRET, 'utf-8')],
...
module.exports = {
  ...
  async addSessionToToken(req, sessionType, instance) {
    ...
    if (token) {
      Object.keys(TOKEN_KEYS)
            ...
            .forEach(key => (newToken[TOKEN_KEYS[key]] = token[TOKEN_KEYS[key]]));
    }
  ...
  async removeSessionFromToken(req, sessionType) {
    const token = await getTokenFromReq(req);
    const otherSessions = Object.keys(TOKEN_KEYS).filter(
      key => (key !== sessionType && token[TOKEN_KEYS[key]])
    );
