...
module.exports = function linkPasses(sequelize, DataTypes) {
    ...
    Pass.associate = function associate(models) {
        ...
        // models.passes.hasOne(models.dibs_transaction, {
        //     foreignKey: 'for_passid',
    ...
    Pass.createNewPass = function createNewPass({
        ...
        let expirationDate =
            studioPackage.passesValidFor && !studioPackage.expires_after_first_booking
                ? moment().tz(studio.mainTZ).endOf('day').add(studioPackage.passesValidFor, studioPackage.validForInterval).toISOString()
                : null;
    ...
    Pass.prototype.returnFromTransaction = async function returnFromTransaction({
        save = false,
        transaction = null,
        associatedTransactionId = null
    } = {}) {
        if (this.source_serviceid) return;
        if (moment(this.expiresAt).isBefore(moment()) && !this.studioPackage.unlimited) {
    ...
    Pass.prototype.setExpirationFromEvent = function setExpirationFromEvent({
      ...
      */
            const startOfValidInterval = moment(this.expiresAt).subtract(
                this.studioPackage.passesValidFor,
                this.studioPackage.validForInterval
            );
            if (eventStart >= startOfValidInterval) return this;
        }
        this.expiration_set_by_transaction = dibsTransactionId;
        this.expiresAt = eventStart.add(this.studioPackage.passesValidFor, this.studioPackage.validForInterval).endOf('day').toDate();
        if (!save) return this;
        return this.save(transaction ? { transaction } : undefined);
    };
    ...
    Pass.prototype.resetExpirationSetByEvent = async function resetExpirationSetByEvent({ save = true, transaction } = {}) {
      ...
      */
            const dibsTransaction = await this.sequelize.models.dibs_transaction.findOne({
                ...
                order: [[{ model: this.sequelize.models.event, as: 'event' }, 'start_date', 'ASC']],
                include: [
                    {
                        model: this.sequelize.models.event,
                        as: 'event',
    ...
    Pass.prototype.isValid = function isPassValid() {
        return this.shouldDisplay() && (!this.totalUses || this.usesCount < this.totalUses || this.autopay);
    };
