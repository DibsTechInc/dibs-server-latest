...
module.exports = function linkDibsUser(sequelize, DataTypes) {
     ...
     */
    const DibsUser = sequelize.define(
        'dibs_user',
        {
            id: {
    ...
    DibsUser.prototype.apiJSON = function apiJSON() {
        return {
            ...
            flash_credits:
                typeof this.flash_credits !== 'undefined'
                    ? this.flash_credits.filter((c) => moment() < moment(c.expiration)).map((c) => c.apiJSON())
                    : []
            // COME BACK TO THIS: make next line work
    ...
    DibsUser.prototype.hasBookedOnDibs = async function hasBookedOnDibs() {
        return (
            (await this.constructor.sequelize.models.dibs_transaction.count({
    ...
    DibsUser.prototype.clientJSON = function clientJSON() {
        ...
        return {
            ...
            passes:
                typeof this.passes !== 'undefined'
                    ? this.passes
                          .filter((p) => !p.expiresAt || moment(p.expiresAt).isAfter(moment()))
                          .map((p) => ({
                              ...p.get(),
                              isValid: p.isValid()
                          }))
    ...
    DibsUser.prototype.addCreditForStudio = async function addCreditForStudio(
        amount,
        studio,
        { save = false, transaction = null, associatedTransactionId = null, creditTier = null, creditTransactionType = null }
    ) {
    ...
    DibsUser.prototype.syncOffsitePasses = async function syncOffsitePasses(studioid, source, { throttle = false } = {}) {
        ...
        const { data } = await axios.put(
            `${host}/api/passes/${studioid}/${this.id}`,
            {},
            {
    ...
    DibsUser.prototype.syncOffsiteClasses = async function syncOffsiteClasses(studioid, source, { throttle = false } = {}) {
        ...
        const host = process.env.NODE_ENV === 'production' ? `https://${source}sync.ondibs.com` : 'http://localhost:3001';
        const { data } = await axios.put(
            `${host}/api/classes/${studioid}/${this.id}`,
            {},
            {
                headers: { Authorization: `Basic ${apiKey}` }
            }
    ...
    DibsUser.prototype.getMemberFixedPrice = async function getMemberFixedPrice(dibsStudioId) {
        ...
        // fix this later
        // const fixedPriceToReturn = (this.fixedPrices[dibsStudioId] = null)
        if (!membershipPasses.length) return { sorting: true };
    ...
    DibsUser.prototype.hasMadePurchaseAtStudio = async function hasMadePurchaseAtStudio(dibsStudioId) {
        const attendeeCount = await this.sequelize.models.attendees.count({
            where: {
                email: { [Op.iLike]: this.email },
                dibs_studio_id: dibsStudioId,
                [Op.not]: {
                    serviceName: {
                        [Op.or]: [{ [Op.iLike]: '%classpass%' }, { [Op.iLike]: '%gilt%' }, { [Op.iLike]: '%fitreserve%' }]
                    }
                },
                dropped: { [Op.or]: [null, false] }
            }
        });
        const pastTransactionsCount = await this.sequelize.models.dibs_transaction.count({
            where: {
                userid: this.id,
                dibs_studio_id: dibsStudioId,
                status: 1,
                ...
                type: {
                    [Op.notIn]: [
                        this.sequelize.models.dibs_transaction.Types.REFER_A_FRIEND_CREDIT,
                        this.sequelize.models.dibs_transaction.Types.COMP_CREDIT
                    ]
                },
    ...
    DibsUser.fullTextSearch = async function fullTextSearch(str, { where = {}, limit = '' } = {}) {
        const additionalWhere = Object.keys(where).reduce((acc, key) => `${acc} AND "${key}" = ${where[key]}`, '');
    ...
    DibsUser.prototype.getUserStudioSpend = async function getUserStudioSpend(dibsStudioId, pastDays = DEFAULT_PAST_DAYS) {
        const [{ spend }] = await this.sequelize.query(
            `SELECT ROUND(SUM(amount - studio_credits_spent - raf_credits_spent)::NUMERIC, 2) as spend
