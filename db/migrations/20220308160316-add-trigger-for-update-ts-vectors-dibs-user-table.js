...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    return Promise.map(tableColumns, async (tc) => {
      ...
      await queryInterface.sequelize.query(`CREATE OR REPLACE FUNCTION update_${tc.table}_ft_search() 
          ...
          $$ LANGUAGE plpgsql;
        `);
        return queryInterface.sequelize.query(`CREATE TRIGGER ${tc.table}_ft_search_update_tgr_w_mobile
  ...
  down: async (queryInterface, Sequelize) => {
      return Promise.map(tableColumns, async (tc) => {
        await queryInterface.sequelize.query(`DROP TRIGGER ${tc.table}_ft_search_update_tgr_w_mobile ON ${tc.table};`)
